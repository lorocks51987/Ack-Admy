...
export function OrderingScreen({ exercise, onAnswer }: Props) {
  ...
  const handleCheck = () => {
    ...
    const correct = arranged.every(
      (entry, i) => entry.origIdx === exercise.correctOrder[i]
    );
    onAnswer(correct);
  };
            ...
            <View style={styles.chips}>
              {arranged.map((entry, i) => (
                <TouchableOpacity
                  ...
                  activeOpacity={0.8}
                >
                  <Text style={[styles.chipNum, { color: "#FFFFFF88" }]}>
                    {i + 1}
                  </Text>
                  <Text style={[styles.chipText, { color: "#FFFFFF" }]}>
                    {entry.item}
                  </Text>
                  <Feather name="x" size={14} color="#FFFFFF88" />
                </TouchableOpacity>
              ))}
            </View>
          )}
